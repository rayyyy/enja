...
2. **キャレット移動**: スナップショットが取れないターゲット向け。AXSelectedTextRange / AXSelectedTextMarkerRange の変化を挿入の証拠とする（Mail 等の WKWebView 本文編集）。マーカーは CFEqual が値比較を実装しているかを 2 回読みで較正し、ポインタ比較に落ちる実装ではチャネル自体を捨てる（偽成功防止）。動いた場合のみ成功と判断する正方向専用チャネル。
...
どのチャネルでも確認できなかった場合、**楽観的に成功扱いにするのは明示的なテキスト入力ロール（AXTextArea 等。Cursor/Monaco/Electron の隠し textarea を含む）だけ**（`allows_unverified_paste`）。web area（属性名はキャレットの有無と無関係に並ぶ）や pid しか分からないターゲットは失敗としてコピー用フォールバックダイアログに倒す。さらに、確認手段がひとつも無く楽観視もできないターゲットには Cmd+V 自体を送らない（クリップボードにも触れない）。「カーソルがテキスト入力に無いときは必ずダイアログを出す」が最優先で、偽ダイアログの削減はその次という優先順位。
