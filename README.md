...
| **Speech recognition options**  | Google Speech-to-Text Chirp 3 / Deepgram Nova-3 / OpenAI gpt-4o transcribe / Gemini 音声入力                                               |
| **License**                     | [MIT](LICENSE)                                                                                                                            |
...
- **音声データ**: 音声入力では録音した音声が、設定した音声認識プロバイダ（Google Speech-to-Text / Deepgram / OpenAI / Gemini）へ送信されます。認識結果は Gemini で自然な文章に整形され、音声指示では選択テキストと指示内容をもとに最終テキストを生成します。
- **API キー / 認証情報**: アプリの設定で保存した Gemini / OpenAI / Deepgram API キーや Google Service Account は macOS Keychain（実装は [`src-tauri/src/secrets.rs`](src-tauri/src/secrets.rs)）に保存され、`settings.json` にはモデル・ショートカット・プロンプトなどの非秘密情報だけを書き込みます。リポジトリや Issue にキーを載せないでください。
...
| 音声認識プロバイダの認証情報                          | Google Chirp 3 / Deepgram / OpenAI / Gemini 音声入力を使う場合 |
